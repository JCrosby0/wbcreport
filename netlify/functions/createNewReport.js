...
// Functions to generate specific shets
const createOverviewSheet = async (doc, payload) => {
...
const createSportsSheet = async (doc, payload) => {
...
const createTeamsSheet = async (doc, payload) => {
  const sheetOutstanding = doc.sheetsByTitle["Outstanding By Team"];
  await sheetOutstanding.resize({ rowCount: 1, columnCount: 4 });
  await sheetOutstanding.addRows(payload.outstanding);
};

const createReportSheet = async (doc, payload) => {
  const d = new Date();
...
exports.handler = async (event, context, callback) => {
  ...
  /**
   * payload comes from front end, and has data in format:
   *  const payload = {
  ...
  // call functions to create sheets
  createOverviewSheet(doc, payload);
  createSportsSheet(doc, payload);
  createTeamsSheet(doc, payload);
  createReportSheet(doc, payload);

  // return message to the frontend
  return returnFunction(null, { message: "Report generation complete" });
};
