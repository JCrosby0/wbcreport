...
export class Reports {
  ...
  constructor(
    ...
    dataArray: ReportLineItem[],
    headingArray: Headings[],
    columns: string[] = ["name", "paymentClass"]
  ) {
    this.label = label;
...
export class ReportLineItem {
  ...
  constructor(
    ...
    if (
      balanceType &&
      ["baseball", "softball", "teeball", "tee-ball"].includes(balanceType)
    ) {
      this.balanceOutstanding = this.dataArray.reduce(
        (a: number, c: PlayerRecord) => {
          return (
            a +
            +c[
              balanceType === "baseball" ? "balanceBaseball" : "balanceSoftball"
            ]
          );
        },
        0
